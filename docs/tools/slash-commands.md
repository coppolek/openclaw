...
- `/emotions on|off|full` toggles the expressive speech layer: `on` lets `voice.md` guide TTS while hiding tags in normal display, `full` preserves visible tags, and `off` disables the layer.
