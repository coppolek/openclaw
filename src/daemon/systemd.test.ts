...
function resolveExpectedRestartMarkerPath(unitName = GATEWAY_SERVICE) {
  return path.join(
    os.tmpdir(),
    `openclaw-systemd-restart-expected-${unitName.replace(/[^a-zA-Z0-9._-]+/g, "-")}.txt`,
  );
}
...
' "$target_unit" "$(date +%s)" > "$marker_path"`);
  expect(args[8]).toContain('exec systemctl --user restart "$target_unit"');
  expect(args[9]).toBe("openclaw-systemd-restart-handoff");
  expect(args[10]).toBe(String(process.pid));
  expect(args[11]).toBe(unitName);
  expect(args[12]).toBe(resolveExpectedRestartMarkerPath(unitName));
}
