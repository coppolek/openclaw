...
async function waitForGatewayPortReady(port: number): Promise<boolean> {
...
async function waitForHealthyGatewayChild(
  ...
  while (Date.now() < deadline) {
    if (await waitForGatewayPortReady(port)) {
      return true;
    }
...
export async function runGatewayLoop(params: {
  ...
  runtime: RuntimeEnv;
  lockPort?: number;
  waitForHealthyChild?: (port: number, pid?: number) => Promise<boolean>;
}) {
  ...
  const handleRestartAfterServerClose = async (restartReason?: string) => {
    ...
    if (isUpdateRestart) {
      ...
      if (respawn.mode === "spawned") {
        ...
        const healthy =
          typeof port === "number" ? await waitForHealthyChild(port, respawn.pid) : false;
        if (healthy) {
          gatewayLog.info(
