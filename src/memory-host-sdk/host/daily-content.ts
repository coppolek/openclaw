...
async function readBoundaryFileContents(fd: number): Promise<string> {
  const chunks: Buffer[] = [];
  let totalBytes = 0;
  for (;;) {
    const buf = Buffer.allocUnsafe(64 * 1024);
    const bytesRead = await new Promise<number>((resolve, reject) => {
      fsSync.read(fd, buf, 0, buf.length, null, (error, read) => {
        if (error) {
          reject(error);
          return;
        }
...
async function readSessionSummaryWorkspaceFile(params: {
  ...
  try {
    return {
      kind: "content",
      raw: await readBoundaryFileContents(opened.fd),
