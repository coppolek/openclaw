...
describe("buildBareSessionResetPrompt", () => {
  it("includes the explicit Session Startup instruction for bare /new and /reset", () => {
    ...
    expect(prompt).toContain(
      "Use the provided Project Context and any runtime-loaded startup context first",
    );
    ...
    expect(prompt).toContain("only read workspace files directly if key context is missing");
  });
