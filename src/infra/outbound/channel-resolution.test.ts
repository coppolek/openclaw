...
describe("outbound channel resolution", () => {
  ...
  it("normalizes already-threaded targets before reusing them as currentChannelId", async () => {
    getLoadedChannelPluginMock.mockReturnValue({
      id: "alpha",
      messaging: {
        normalizeTarget: (raw: string) => (raw.startsWith("group:") ? `telegram:${raw}` : raw),
        parseExplicitTarget: ({ raw }: { raw: string }) => {
          const topicMatch = /^(.*):topic:(\d+)$/u.exec(raw);
          if (topicMatch) {
            return {
              to: topicMatch[1],
    ...
    expect(
      channelResolution.resolveOutboundCurrentChannelTarget({
        channel: "alpha",
        to: "group:-1001234567890:topic:42",
        threadId: 42,
        cfg: {} as never,
      }),
    ).toBe("telegram:group:-1001234567890:topic:42");
  });
