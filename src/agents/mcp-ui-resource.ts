...
export function extractMcpAppResourceUri(value: unknown): string | undefined {
  const record = getRecord(value);
  const meta = getRecord(record?._meta);
  const ui = getRecord(meta?.ui);
  const rawUri = ui?.resourceUri ?? meta?.["ui/resourceUri"];
  return typeof rawUri === "string" && rawUri.startsWith("ui://") ? rawUri : undefined;
}
...
export async function fetchMcpAppView(params: {
  ...
  try {
    ...
    if (
      ...
      content.mimeType !== MCP_APP_RESOURCE_MIME_TYPE
    ) {
      logWarn(
        `mcp-app: unsupported resource mime type ${content.mimeType} for ${params.uiResourceUri} from server "${params.serverName}"`,
      );
      return undefined;
    }
    const html =
