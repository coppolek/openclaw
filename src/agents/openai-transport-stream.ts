...
import { planResponsesTurnInput } from "./openai-responses-message-conversion.js";
import {
...
function getOrCreatellmSession(sessionId: string, provider: string): llmSession {
  let session = llmSessionRegistry.get(sessionId);

  if (!session) {
    llmSessionRegistry.clear();
  }
  ...
  if (!session || session.provider !== provider) {
    ...
    llmSessionRegistry.set(sessionId, session);
  }
  return session;
}
...
export function createOpenAIResponsesTransportStreamFn(): StreamFn {
  return (model, context, options) => {
    ...
    void (async () => {
      ...
      const providerOptions = (model as { providerOptions?: Record<string, unknown> })
        .providerOptions;
      const llmStateful = (providerOptions?.llmStateful as boolean | undefined) ?? false;

      let session: llmSession | null = null;
      if (llmStateful && options?.sessionId) {
        session = getOrCreatellmSession(options.sessionId, model.provider);
      } else if (!llmStateful) {
        llmSessionRegistry.clear();
      }
