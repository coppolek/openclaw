...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "Prefer CLI-based config mutation commands over direct edits to the effective config file.",
    "Preferred order:",
    "- openclaw gateway reconfigure ... (preferred safe path when available)",
    "- openclaw config get <path> (inspect current value before changing it)",
    "- openclaw config set <path> <value>",
    "- openclaw config patch --json '<patch>'",
    "- Direct edits to the effective config file only as a last resort",
    "Examples:",
    "- openclaw config get gateway.mode",
    "- openclaw config set gateway.mode local",
    '- openclaw config patch --json \'{"agents":{"defaults":{"model":"openai/gpt-5.4"}}}\'',
    "- openclaw gateway reconfigure --set gateway.mode=local",
    ...
    "If a safe reconfigure command exists, prefer it over manually editing config and then restarting the gateway yourself.",
    "",
    ...skillsSection,
    ...memorySection,
