...
describe("browser client", () => {
  ...
  it("uses the expected endpoints + methods for common calls", async () => {
    ...
    const act = calls.find(
      (c) => c.url.endsWith("/act") && c.init?.body === JSON.stringify({ kind: "wait", timeMs: 20_000 }),
    );
    expect((act?.init as (RequestInit & { timeoutMs?: number }) | undefined)?.timeoutMs).toBe(25_000);
