...
export function isProfileDecorated(
  ...
  const desiredDownloadDir = opts?.downloadDir?.trim();
  ...
  const downloadPrefs = (() => {
    const download = prefs?.download;
    return typeof download === "object" && download !== null && !Array.isArray(download)
      ? (download as Record<string, unknown>)
      : null;
  })();

  const savefilePrefs = (() => {
    const savefile = prefs?.savefile;
    return typeof savefile === "object" && savefile !== null && !Array.isArray(savefile)
      ? (savefile as Record<string, unknown>)
      : null;
  })();
  ...
  const downloadOk =
    !desiredDownloadDir ||
    (downloadPrefs?.default_directory === desiredDownloadDir &&
      downloadPrefs?.prompt_for_download === false &&
      downloadPrefs?.directory_upgrade === true &&
      savefilePrefs?.default_directory === desiredDownloadDir);
...
export function decorateOpenClawProfile(
  ...
  if (desiredDownloadDir) {
    // Seed both automatic-download and Save As locations. On macOS, some Chrome
    // flows consult savefile.default_directory even for downloads that end up in
    // chrome://downloads, and missing prefs can leave entries half-baked/unusable.
    setDeep(prefs, ["download", "default_directory"], desiredDownloadDir);
    setDeep(prefs, ["download", "prompt_for_download"], false);
    setDeep(prefs, ["download", "directory_upgrade"], true);
    setDeep(prefs, ["savefile", "default_directory"], desiredDownloadDir);
  }
