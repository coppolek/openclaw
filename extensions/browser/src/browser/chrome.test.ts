...
describe("browser chrome profile decoration", () => {
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    expect(download?.default_directory).toBeUndefined();
    expect(savefile?.default_directory).toBeUndefined();
  ...
  it("writes download prefs when an explicit download dir is provided", async () => {
    const userDataDir = await createUserDataDir();
  ...
  it("treats missing download prefs as undecorated when a download dir is required", async () => {
    const userDataDir = await createUserDataDir();
    decorateOpenClawProfile(userDataDir, { color: DEFAULT_OPENCLAW_BROWSER_COLOR });
