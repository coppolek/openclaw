...
describe("handleFeishuMessage DM topic session scoping", () => {
  ...
  it("routes /reset notification into DM topic via MessageThreadId", async () => {
    const cfg: ClawdbotConfig = {
      channels: { feishu: { dmPolicy: "open" } },
    } as ClawdbotConfig;

    const event: FeishuMessageEvent = {
      sender: { sender_id: { open_id: "ou_dm_reset_user" } },
      message: {
        message_id: "msg-dm-reset",
        root_id: "om_dm_root_reset",
        chat_id: "oc-dm-chat",
        chat_type: "p2p",
        message_type: "text",
    ...
    // The finalizeInboundContext should include MessageThreadId for DM topic
    expect(mockFinalizeInboundContext).toHaveBeenCalledWith(
      expect.objectContaining({
        MessageThreadId: "om_dm_root_reset",
      }),
    );
  });
});
